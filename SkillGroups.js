...
const RAYMADEIT = RAYMADEIT || {};
RAYMADEIT.SkillGroups = RAYMADEIT.SkillGroups || {};
RAYMADEIT.NotetagGetters = RAYMADEIT.NotetagGetters || [];

(function (_) {
  'use strict';

  // ---------------------------------------------------------------------------
  // RAYMADEIT.SkillGroups
  // ---------------------------------------------------------------------------
  ...
  RAYMADEIT.NotetagGetters.push(_.loadNotetags);

  // ---------------------------------------------------------------------------
  // DataManager
  // ---------------------------------------------------------------------------

  if (!RAYMADEIT.DataManager_isDatabaseLoaded) {
    RAYMADEIT.notetagsLoaded = false;
    RAYMADEIT.DataManager_isDatabaseLoaded = DataManager.isDatabaseLoaded;
    DataManager.isDatabaseLoaded = function () {
      if (!RAYMADEIT.DataManager_isDatabaseLoaded.apply(this, arguments)) {
        return false;
      }
      if (!RAYMADEIT.notetagsLoaded) {
        RAYMADEIT.NotetagGetters.forEach(function (func) {
          func.call(this);
        }, this);
        RAYMADEIT.notetagsLoaded = true;
      }
      return true;
    };
  }
  ...
  Window_ActorCommand.prototype.makeCommandList = function () {
    ...
    if (actorData._se_commandOrder) {
      this.addCommand('Escape', 'escape', /* enabled= */ true);
